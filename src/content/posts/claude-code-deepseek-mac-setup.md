...
description: '從零開始，在 Mac 終端機安裝 Claude Code，並串接中國 DeepSeek API 做為程式開發助手——最適合中文使用者的 CI 級 AI 開發環境設定指南'
date: 2026-06-11
tags: ['AI', 'Claude', 'DeepSeek', 'Mac', '開發工具', '教學']
draft: false
---

如果你是一個開發者，最近一定被「AI 寫程式」的新聞轟炸。Claude Code、GitHub Copilot、Cursor——但你可能不知道的是，這些工具大多能**自己選擇模型**，不一定要綁在美國的 AI 服務上。
...
Mac 的終端機在 `應用程式 → 工具程式 → 終端機`，或者直接按 `Cmd + 空白鍵` 搜尋「終端機」。

打開後你會看到一個黑底白字的視窗，顯示類似：
...
跑完之後，關掉終端機再重開（`Cmd + Q` 完全退出再打開），或者先執行這行讓設定立刻生效：
...
Claude Code 是 Anthropic 推出的命令列 AI 程式設計工具，但它的設計比你想像的更開放——你可以讓它用**任何相容的 API**。
...
Claude Code 預設會用 Anthropic 的官方 API（需要美國手機驗證，對中國用戶不友善）。但我們可以透過一個代理層，讓它串接到 DeepSeek 的 API。
...
![DeepSeek API 金鑰建立頁面](https://images.unsplash.com/photo-1633356122102-3fe601e05bd2?q=80&w=2670&auto=format&fit=crop)

*在 DeepSeek 平台建立你的第一組 API Key*
...
Claude Code 支援 `ANTHROPIC_BASE_URL` 環境變數來改變 API 端點。我們需要一個**轉發層**，因為 Claude Code 的通訊格式跟 DeepSeek 不完全相同。

你可以用 **one-api** 或 **new-api** 這樣的開源專案來做轉發。最簡單的方式是用一個輕量的 Python 腳本：
...
### 實戰測試：請 AI 幫你寫一個擷取網頁標題的腳本

在 Claude Code 的提示輸入：

> 幫我寫一個 Python 腳本，接收一個 URL 參數，下載該網頁的 HTML 並提取 `<title>` 標籤的內容。輸出格式為「網頁標題: xxx」。
...
- **設定 max_tokens**：回答太長時，Claude Code 支援傳入 `--max-tokens` 參數限制輸出長度
...
當你能在終端機中直接對 AI 下達「幫我寫一個爬蟲」、「解釋這段程式碼」、「重構這個函數」時，你才會真正感受到 AI 程式設計助手的威力。

---

*下一篇預告：將 Claude Code 整合進 VS Code，在編輯器中直接與 AI 對話*
